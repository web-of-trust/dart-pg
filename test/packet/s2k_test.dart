...
void main() {
  group('s2k', () {
    test('mode-0-password-1234 test', () async {
      ...
      final key =
          await s2k.produceKey(passphrase, (skesk.symmetric.keySize + 7) >> 3);
      expect(key.toHexadecimal(),
    ...
    test('mode-1-password-123456 test', () async {
      ...
      final salt =
          Uint8List.fromList([0xa8, 0x42, 0xa7, 0xa9, 0x59, 0xfa, 0x42, 0x2a]);

      final packets =
          PacketList.packetDecode(base64.decode(mode1Password123456));
      final skesk = packets[0] as SymEncryptedSessionKeyPacket;
    ...
    test('mode-1-password-foobar test', () async {
      ...
      final salt =
          Uint8List.fromList([0xbc, 0x95, 0x58, 0x45, 0x81, 0x3c, 0x7c, 0x37]);

      final packets =
          PacketList.packetDecode(base64.decode(mode1PasswordFoobar));
      final skesk = packets[0] as SymEncryptedSessionKeyPacket;
      expect(skesk.s2k.type, S2kType.salted);
      expect(skesk.s2k.hash, HashAlgorithm.sha1);
      expect(skesk.s2k.salt, equals(salt));
    ...
    test('mode-3-password-qwerty test', () async {
      ...
      final salt =
          Uint8List.fromList([0x78, 0x45, 0xf0, 0x5b, 0x55, 0xf7, 0xb4, 0x9e]);

      final packets =
          PacketList.packetDecode(base64.decode(mode3PasswordQwerty));
      final skesk = packets[0] as SymEncryptedSessionKeyPacket;
    ...
    test('mode-3-password-9876 test', () async {
      ...
      final salt =
          Uint8List.fromList([0xb9, 0x67, 0xea, 0x96, 0x53, 0xdb, 0x6a, 0xc8]);
    ...
    test('mode-3-aes192-password-123 test', () async {
      ...
      final salt =
          Uint8List.fromList([0x8f, 0x81, 0x74, 0xc5, 0xd9, 0x61, 0xc7, 0x79]);

      final packets =
          PacketList.packetDecode(base64.decode(mode3Aes192Password123));
      final skesk = packets[0] as SymEncryptedSessionKeyPacket;
      expect(skesk.s2k.type, S2kType.iterated);
      expect(skesk.s2k.hash, HashAlgorithm.sha1);
      expect(skesk.s2k.itCount, itCount);
    ...
    test('mode-3-twofish-password-13-times-0123456789 test', () async {
      ...
      final salt =
          Uint8List.fromList([0x51, 0xed, 0xfc, 0x15, 0x45, 0x40, 0x65, 0xac]);

      final packets = PacketList.packetDecode(
          base64.decode(mode3TwofishPassword13Times0123456789));
      final skesk = packets[0] as SymEncryptedSessionKeyPacket;
      expect(skesk.s2k.type, S2kType.iterated);
      expect(skesk.s2k.hash, HashAlgorithm.sha1);
      expect(skesk.s2k.itCount, itCount);
    ...
    test('mode-3-aes128-password-13-times-0123456789 test', () async {
      ...
      const passphrase =
          '0123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789';
      final salt =
          Uint8List.fromList([0x06, 0xe4, 0x61, 0x5c, 0xa4, 0x48, 0xf9, 0xdd]);
