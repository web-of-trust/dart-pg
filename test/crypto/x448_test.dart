  ...
  group('Test X448 ECDH', () {
    ...
    test('Test ECDH', () {
      for (var i = 0; i < 100; i++) {
        ...
        final qB = X448.scalarMultBase(kB);

        /// Compute the shared secrets
        final sA = X448.scalarMult(kA, qB);
        final sB = X448.scalarMult(kB, qA);

        expect(sA, equals(sB), reason: 'Shared secrets did not match');
      }
    });
  });

  group('Test X448 Vector', () {
    checkX448Vector(
...
void checkX448Vector(String sk, String su, String se, String text) {
  test(text, () {
    final k = sk.hexToBytes();
    final u = su.hexToBytes();
    final sr = X448.scalarMult(k, u);
    expect(sr, equals(se.hexToBytes()), reason: 'Vector did not match');
  });
}
...
void checkECDHVector(String sA, String sAPub, String sB, String sBPub, String sK, String text) {
  test(text, () {
    ...
    final bK = X448.scalarMult(b, aPub);
    expect(bK, equals(sK.hexToBytes()), reason: 'Shared secret did not match');
  });
}
